...
def visualize(df: pd.DataFrame, outdir: Path):
    ...
    ax11.plot(x, df[ID_CTS], label=ID_CTS, color=colors[0], linestyle="-")
    ax11.plot(x, df[ID_CW],  label=ID_CW,  color=colors[1], linestyle="-")
    ax11.plot(x, df[ID_CC],  label=ID_CC,  color=colors[2], linestyle="-")
    ...
    ax12 = ax11.twinx()
    ax12.plot(x, df[ID_PROP_CG], label=ID_PROP_CG, color=colors[0], linestyle="--")
    ax12.plot(x, df[ID_PROP_CW], label=ID_PROP_CW, color=colors[1], linestyle="--")
    ax12.plot(x, df[ID_PROP_CC], label=ID_PROP_CC, color=colors[2], linestyle="--")
    ...
    # left axis (solid lines)
    ax2.plot(x, df[ID_RWS], label="relative warehouse savings (%)", color=colors[1], linestyle="--")
    ax2.plot(x, df[ID_RCS],  label="relative carrier savings (%)",  color=colors[2], linestyle="--")
    ax2.axhline(y=df[ID_RCS].max(), color="red", linestyle="--", linewidth=1.5, label=f"Upper bound on carrier savings: {df[ID_RCS].max():.2f} (%)")
    ...
    ax31.plot(x, df[ID_LPV], label="Levelized parcel value (EUR)", color=colors[0], linestyle="-")
    # vertical line at subscription size = 110
    ax31.axhline(y=df[ID_LPV].max(), color="red", linestyle="--", linewidth=1.5, label=f"Upper bound on parcel value: {df[ID_LPV].max():.0f} (EUR/parcel)")
